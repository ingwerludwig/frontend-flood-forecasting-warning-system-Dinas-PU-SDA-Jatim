...
const UserTable = ({user}) => {
    ...
    const handleOpenModal = (state, userinfo=null) => {
        setOpenModal(true)
        if (state === 'add') {
            setModalType(<Modal component={<FormUser nama={null} nohp={null} apikey={null} type={"TAMBAH"} handler={handleAddKontak} error={error}/>} close={() => setOpenModal(false)}
            message="Masukkan informasi pada form di bawah" modalTitle="Tambah Kontak"/>)
        } else {
            setModalType(<Modal component={<FormUser nama={userinfo.nama} nohp={userinfo.nohp} apikey={userinfo.apikey} id={userinfo.id} type={"UPDATE"} handler={handleUpdateKontak} error={error}/>}
            close={() => setOpenModal(false)}
            message="Masukkan informasi pada form di bawah" modalTitle="Update Kontak"/>)
        }
    }

    const handleAddKontak = async (nama, no_hp, apikey) => {
        const res = await addKontak(user.authorization.token, nama, no_hp, apikey);
        if (res) {
            setUpdate(true);
            setOpenModal(false);
            return "sucess"
    ...
    const handleUpdateKontak = async (nama, no_hp, apikey, id) => {
        const res = await updateKontak(user.authorization.token, nama, no_hp, apikey, id)
        if (res) {
            setUpdate(true);
            setOpenModal(false);
            return "sucess"
    ...
    return (
        <div className="text-left">
            ...
            <div className="grid grid-cols-10 my-3 border rounded-lg shadow text-center">
                ...
                <div className="col-span-2 font-semibold py-2 text-sm border-b">API Key</div>
                <div className="col-span-1 font-semibold py-2 text-sm border-b">Aksi</div>
                ...
                {users.map(item => (
                    <div className="col-span-10 grid grid-cols-10" key={item.id}>
                        ...
                        <div className="col-span-2 border-b text-sm py-1">{item.apiKey}</div>
                        <div className="col-span-1 border-b text-sm py-1 flex-wrap flex justify-center">
                            <button onClick={() => handleOpenModal('update', {nama:item.nama_kontak, nohp:item.no_telp, apikey:item.apiKey, id: item.id})}
...
const FormUser = ({nama, nohp, apikey, id, type, handler}) => {
    const [name, setName] = useState(nama ? nama: '')
    const [hp, setHp] = useState(nohp ? nohp : '')
    const [apikeys, setApikey] = useState(apikey ? apikey : '')
    const [error, setError] = useState(null);

    const handleSubmit = async () => {
        let res = null;
        if (handler && type === 'TAMBAH') {
            res = await handler(name, hp, apikeys);
        } else if (handler && type === 'UPDATE') {
            res = await handler(name, hp, apikeys, id)
        }
        if (res !== 'sucess') {
            setError(res)
        }
    ...
    return (
        <div className="">
            <form className="my-3">
                ...
                <input value={apikeys} onChange={(e) => setApikey(e.target.value)} type="text" className="border border-black px-3 py-1 rounded-lg w-full"/>
            </form>
